...
def upload_graph_page(request):
    ...
    if request.method == 'POST':
        try:
            ...
            context['Success'] = settings.URL_PATH + \
                "graphs/" + str(graph['id'])
            ...
            if request.POST.get('owner_email', None) is not None:
                ...
                producer.send_message('owner', {
                    ...
                    'message': settings.NOTIFICATION_MESSAGE['owner']['upload']['graph'].format(name=graph.get('name', '')),
                    'resource': 'graph',
...
def graph_page_by_name(request, email, graph_name):
    ...
    graph_name : string
            Name of the Graph.
    """
    graph = graphs.get_graph_by_name(
        request, owner_email=email, name=graph_name)
...
def _update_graph(request, graph_id, graph={}):
    ...
    if graph.get('owner_email', None) is not None:
        producer.send_message('owner', {
            'owner_email': graph.get('owner_email', None),
            'message': settings.NOTIFICATION_MESSAGE['owner']['update']['graph'].format(name=graph.get('name', None)),
            'resource': 'graph',
...
def _delete_graph(request, graph_id):
    ...
    producer.send_message('owner', {
        ...
        'message': settings.NOTIFICATION_MESSAGE['owner']['delete']['graph'].format(name=return_value.get('name', None)),
        'resource': 'graph',
        'resource_id': graph_id,
        'type': 'delete'
    })
...
def _graph_groups_api(request, graph_id, group_id=None):
    ...
    if request.META.get('HTTP_ACCEPT', None) == 'application/json':
        if graph_id is None:
            raise BadRequest(
                request, error_code=ErrorCodes.Validation.GraphIDMissing)
...
def _add_graph_group(request, graph_id, group={}):
    ...
    """
    authorization.validate(
        request, permission='GRAPH_SHARE', graph_id=graph_id)
...
def _graph_layouts_api(request, graph_id, layout_id=None):
    ...
    if request.META.get('HTTP_ACCEPT', None) == 'application/json':
        if request.method == "GET" and layout_id is None:
            ...
            return HttpResponse(
                json.dumps(_update_layout(request, graph_id,
                                          layout_id, layout=json.loads(request.body))),
                content_type="application/json",
                status=200)
...
def _get_layouts(request, graph_id, query=dict()):
    ...
    if user_role == authorization.UserRole.LOGGED_IN:
        if get_request_user(request) != query.get('owner_email', None) \
            ...
            raise BadRequest(
                request, error_code=ErrorCodes.Validation.NotAllowedLayoutAccess, args=get_request_user(request))
...
def _get_layout(request, graph_id, layout_id):
    ...
    """
    authorization.validate(
        request, permission='LAYOUT_READ', layout_id=layout_id)
...
def _add_layout(request, graph_id, layout={}):
    ...
    if layout.get('owner_email', None) is not None:
        ...
        producer.send_message('owner', {
            ...
            'message': settings.NOTIFICATION_MESSAGE['owner']['create']['layout'].format(name=layout.get('name', '')),
            'resource': 'layout',
...
def _update_layout(request, graph_id, layout_id, layout={}):
    ...
    if layout.get('is_shared', None) == 1:
        ...
        producer.send_message('group', {
            'group_ids': group_ids,   
            'message': settings.NOTIFICATION_MESSAGE['group']['share']['layout'].format(name=return_value.get('name','')),
            'resource': 'layout',
        ...
        producer.send_message('group', {
            'group_ids': group_ids,   
            'message': settings.NOTIFICATION_MESSAGE['group']['unshare']['layout'].format(name=return_value.get('name','')),
            'resource': 'layout',
            'resource_id': layout_id,
            'type': 'unshare',
            'owner_email': request.session.get('uid', None)
...
def _add_edge(request, graph_id, edge={}):
    ...
    return utils.serializer(graphs.add_edge(request,
                                            ...
                                            head_node_id=edge.get(
                                                'head_node_id', None),
                                            tail_node_id=edge.get(
                                                'tail_node_id', None),
                                            is_directed=edge.get(
                                                'is_directed', 0),
                                            graph_id=graph_id))
