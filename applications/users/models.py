...
class PasswordResetCode(IDMixin, TimeStampMixin, Base):
    ...
    email = Column(String, ForeignKey('user.email', ondelete="CASCADE",
                                      onupdate="CASCADE"), nullable=False, index=True)
    code = Column(String, nullable=False)
    user = relationship(
        "User", back_populates="password_reset_codes", uselist=False)

    constraints = (UniqueConstraint('email', 'code',
                                    name='_password_reset_code_uc_email_code'),)
    indices = ()
