...
def join_group_page(request, group_id):
    ...
    if 'GET' == request.method:
        ...
        if group is not None and group.invite_code == request.GET.get('code', None):
            if request.session['uid'] is None:
                ...
                try:
                    ...
                    producer.send_message('group', {
                        'group_id': group_id,
                        'message': settings.NOTIFICATION_MESSAGE['group']['add']['member'].format(name=request.session['uid']),
            ...
            # TODO: change it to signup page. Currently we dont have a signup
            # link.
            return redirect('/')
        ...
        if group is not None and group.invite_code == request.POST.get('code', None):
            try:
                if request.session['uid'] is None:
                    ...
                    producer.send_message('group', {
                        'group_id': group_id,
                        'message': settings.NOTIFICATION_MESSAGE['group']['add']['member'].format(name=request.POST.get('user_id', '')),
                        'resource': 'group_member',
                        'resource_id': group_member['user_id'],
                        'type': 'add'
                    })
...
def _update_group(request, group_id, group={}):
    ...
    authorization.validate(
        request, permission='GROUP_UPDATE', group_id=group_id)
...
def _delete_group(request, group_id):
    ...
    """
    authorization.validate(
        request, permission='GROUP_DELETE', group_id=group_id)
...
def _delete_group_member(request, group_id, member_id):
    ...
    group_member = utils.serializer(
        users.get_user_by_id(request, id=member_id))
...
def _get_group_graphs(request, group_id):
    ...
    total, graphs = users.search_group_graphs(request,
                                              group_id=group_id,
                                              owner_email=request.GET.get(
                                                  'owner_email', None),
                                              names=names if names is None or isinstance(names, list) else [
                                                  names],
                                              nodes=nodes if nodes is None or isinstance(nodes, list) else [
                                                  nodes],
                                              edges=edges if edges is None or isinstance(edges, list) else [
                                                  edges],
...
def _add_group_graph(request, group_id):
    ...
    ------
    """
    authorization.validate(request, permission='GRAPH_SHARE',
