...
def approve_task_expert(request):
	...
	for task in tasks:
		if task.submitted == 0:
			...
			context = db.set_task_layout_context(request, context, uid, gid, layout.layout_name, layout.owner_id,
			                                     approve=True, expert=True)
...
def submitEvaluation(request):
	...
	if request.POST:
		...
		task_code = db.submitEvaluation(uid, gid, layout_name, layout_owner, triangle_rating, rectangle_rating,
		                                shape_rating, color_rating, hit_id)

		if task_code != None:
			return HttpResponse(json.dumps(db.sendMessage(201, task_code)), content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.throwError(500, "Evaluation Submission Unsucessful!")),
			                    content_type="application/json")
	else:
...
def submitExpertEvaluation(request):
	...
	if request.POST:
		...
		task_code = db.submitEvaluation(uid, gid, layout_name, layout_owner, triangle_rating, rectangle_rating,
		                                shape_rating, color_rating, hit_id, expert=True)
...
def retrieveTaskCode(request):
	...
	if request.POST:
		...
		if not gid or not uid:
			return HttpResponse(
				json.dumps(db.throwError(201, "Must include both graph_id and user_id in POST request.")),
				content_type="application/json")
...
def view_json(request, uid, gid):
	...
	if db.is_public_graph(uid, gid) or 'Public_User_' in uid:
		...
		context[
			'Error'] = "You are not authorized to view JSON for this graph, create an account and contact graph's owner for permission to see this."
		return render(request, 'graphs/error.html', context)
	else:
		# If the user is member of group where this graph is shared
		user_is_member = db.can_see_shared_graph(context['uid'], uid, gid)
		...
		if request.session['uid'] == uid or user_is_member == True:
			...
			if graph_info != None:
				...
				context[
					'Error'] = "Graph: " + gid + " does not exist for " + uid + ".  Upload a graph with this name into GraphSpace in order to see it's JSON."
				return render(request, 'graphs/error.html', context)
		else:
			context[
				'Error'] = "You are not authorized to view JSON for this graph, please contact graph's owner for permission."
			return render(request, 'graphs/error.html', context)
	...
	if uid is not None:
		...
		# if admin, then they can view this
			...
			if db.is_admin(uid) == 1:
				...
				context[
					'Error'] = "You are not authorized to see this group's contents! Please contact group's owner to add you to the group!"
				return render(request, 'graphs/error.html', context)
		...
		if view_type == 'owner of' and context['my_groups'] == 0:
			context[
				'message'] = "It appears that you are not an owner of any group.  Please create a group in order to own a group."
		elif view_type == 'member' and context['member_groups'] == 0:
			context[
				'message'] = "It appears as if you are not a member of any group. Please join a group in order for them to appear here."
		else:
	...
	if "uid" in context:
		if group_id != 'all' or group_id != 'member':
			...
			if not any(g_dict['groupId'] == group_id for g_dict in group_dict):
				context[
					'Error'] = "You need to be a member of a group to see its contents!  Please contact group's owner to add you to the group!"
				return render(request, 'graphs/error.html', context)
			...
			if len(graph_data) != 0:
				...
				if context['search_result'] == True:
					context[
						'message'] = "It appears as if there are no graphs in this group that match your search query!"
				else:
...
def register(request):
	...
	if request.method == 'POST' and 'user_id' in request.POST and 'password' in request.POST:
		...
		if register_form.is_valid():
			...
			if user_id == None:
				return HttpResponse(json.dumps(db.throwError(400, "Email already exists!")),
				                    content_type="application/json");
...
def retrieveIDs(request):
	...
	if request.POST:
		...
		for element in element_values:
			...
			# Find an edge
			if ':' in element:
				elementDictionary[element] += db.find_edge(request.POST['uid'], request.POST['gid'], element.strip(),
				                                           request.POST['search_type'])
			else:
				elementDictionary[element] += db.find_node(request.POST['uid'], request.POST['gid'], element.strip(),
				                                           request.POST['search_type'])
...
def sendResetEmail(request):
	...
	if request.POST:
		...
		if emailId == None:
			return HttpResponse(json.dumps(db.throwError(404, "Email does not exist!")),
			                    content_type="application/json")
...
def resetLink(request):
	...
	if request.method == 'GET':
		...
		return HttpResponse(json.dumps(db.throwError(500, "This route only accepts GET requests.")),
		                    content_type="application/json")
...
def resetPassword(request):
	...
	if request.method == "POST":
		...
		if resetInfo == None:
			return HttpResponse(json.dumps(db.throwError(500, "Password Update not successful!")),
			                    content_type="application/json");

		return HttpResponse(json.dumps(db.sendMessage(200, "Password updated for " + request.POST['email'])),
		                    content_type="application/json");
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def launchTask(request):
	...
	return HttpResponse(json.dumps(db.sendMessage(201, "Task Launched on Amazon Mechanical Turk!")),
	                    content_type="application/json");
...
def changeLayoutName(request):
	...
	if request.method == 'POST':
		...
		if loggedIn == None:
			return HttpResponse(json.dumps({"StatusCode": 500, "Message": "Must be logged in to make those requests",
			                                "url": URL_PATH + 'graphs/' + uid + '/' + gid + '/?layout=' + new_layout_name + "&layout_owner=" + loggedIn}),
			                    content_type="application/json")

		error = db.changeLayoutName(uid, gid, old_layout_name, new_layout_name, loggedIn)
		if error == None:
			return HttpResponse(json.dumps({"StatusCode": 200, "Message": "Layout name changed!",
			                                "url": URL_PATH + 'graphs/' + uid + '/' + gid + '/?layout=' + new_layout_name + "&layout_owner=" + loggedIn}),
			                    content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.throwError(400, error)), content_type="application/json")
...
def deleteLayout(request):
	...
	if request.method == 'POST':
		...
		if uid == None:
			return HttpResponse(json.dumps(db.throwError(500, "Must be signed in to delete a layout!")),
			                    content_type="application/json")
		...
		if result == None:
			return HttpResponse(json.dumps(
				{"StatusCode": 200, "Message": "Layout deleted!", "url": URL_PATH + 'graphs/' + uid + '/' + gid}),
			                    content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.throwError(400, result)), content_type="application/json")
	else:
...
def makeLayoutPublic(request):
	...
	if request.method == 'POST':
		...
		if current_user == None:
			return HttpResponse(json.dumps(db.throwError(500, "Must be signed in to make share layouts!")),
			                    content_type="application/json")
		...
		if current_user != uid and db.get_layout_for_graph(layoutId, layout_owner, gid, uid, current_user) == None:
			return HttpResponse(json.dumps(db.throwError(500, "Not authorized to share layouts!")),
			                    content_type="application/json")

		if uid == None:
			return HttpResponse(json.dumps(db.throwError(500, "Must be signed in to make a layout public!")),
			                    content_type="application/json")

		db.makeLayoutPublic(uid, gid, layoutToMakePpublic, loggedIn)
		return HttpResponse(json.dumps({"StatusCode": 200, "Message": "Layout made public!",
		                                "url": URL_PATH + 'graphs/' + uid + '/' + gid + '/?layout=' + new_layout_name}),
		                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def getGroupsForGraph(request):
	...
	if request.method == 'POST':
		...
		if owner == None:
			return HttpResponse(json.dumps(db.throwError(500, "Must be signed in to see groups for this graph!")),
			                    content_type="application/json")

		return HttpResponse(json.dumps(
			{"StatusCode": 200, "Group_Information": db.get_all_groups_for_user_with_sharing_info(owner, gid)}),
		                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def create_group(request, groupname):
	...
	if request.method == 'POST':
		...
		if group_created != None:
			return HttpResponse(json.dumps(
				{"StatusCode": 201, "Message": "Group created!", "Group Name": group_created[0],
				 "Group Id": group_created[1]}, indent=4, separators=(',', ': ')), content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.throwError(400, "Group name already exists for this account"), indent=4,
			                               separators=(',', ': ')), content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def deleteGraph(request):
	...
	if request.method == 'POST':
		...
		if uid == None:
			return HttpResponse(json.dumps(db.throwError(401, "You are not allowed to delete this graph"), indent=4,
			                               separators=(',', ': ')), content_type="application/json")
		...
		if graph_info == None:
			return HttpResponse(
				json.dumps(db.throwError(404, "You do not own any such Graph."), indent=4, separators=(',', ': ')),
				content_type="application/json")
		else:
			...
			if jsonData != None:
				...
				return HttpResponse(
					json.dumps(db.sendMessage(200, "Successfully deleted " + gid + " owned by " + uid + '.'), indent=4,
					           separators=(',', ': ')), content_type="application/json")
			else:
				return HttpResponse(
					json.dumps(db.throwError(404, "You do not own any such Graph."), indent=4, separators=(',', ': ')),
...
def delete_group_through_ui(request):
	...
	if request.method == 'POST':
		if request.POST['username'] == request.POST['groupOwner']:
			...
			return HttpResponse(json.dumps(
				db.sendMessage(200, request.POST['groupName'] + " deleted for " + request.POST['groupOwner'])),
			                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def unsubscribe_from_group(request):
	...
	# If request is a POST request, add it to the server
	if request.method == 'POST':
		result = db.remove_user_through_ui(request.POST['username'], request.POST['groupOwner'],
		                                   request.POST['groupName'])
		if result != None:
			return HttpResponse(json.dumps(db.throwError(400, result)), content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.sendMessage(200, "You are no longer following " + request.POST[
				'groupName'] + " owned by " + request.POST['groupOwner'])), content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def change_description_through_ui(request):
	...
	# If request is a POST request, add it to the server
	if request.method == 'POST':
		result = db.change_description(request.POST['username'], request.POST['groupId'], request.POST['groupOwner'],
...
def getGroupsWithLayout(request):
    ...
    '''
	if request.method == 'POST':
		result = db.is_layout_shared(request.POST['layout'], request.POST['loggedIn'], request.POST['owner'],
		                             request.POST['gid'])
		return HttpResponse(json.dumps({"StatusCode": 200, "Group_Information": result}),
		                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def setDefaultLayout(request):
	if request.method == 'POST':
		...
		if result != None:
			...
			return HttpResponse(json.dumps(db.sendMessage(200, "Set " + request.POST['layoutId'] + " as default")),
			                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def removeDefaultLayout(request):
	if request.method == 'POST':
		...
		if result != None:
			...
			return HttpResponse(json.dumps(db.sendMessage(200, "Removed " + request.POST['layoutId'] + " as default")),
			                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def shareLayoutWithGroups(request):
	...
	if request.method == 'POST':
		...
		if db.can_see_shared_graph(current_user, uid, gid) == None:
			return HttpResponse(json.dumps(db.throwError(500, "Not allowed to do this operation!")),
			                    content_type="application/json")

		if len(db.get_all_groups_for_this_graph(uid, gid)) == 0:
			return HttpResponse(json.dumps(db.throwError(400,
			                                             "No groups to share with.  Either share this graph with a group first or make this graph public!")),
			                    content_type="application/json")
		else:
...
def graph_exists(request, user_id, graphname):
	...
	if request.method == 'POST':
		...
		if request.POST['username'] != user_id:
			return HttpResponse(json.dumps(db.usernameMismatchError(), indent=4, separators=(',', ': ')),
			                    content_type="application/json")

		if db.get_valid_user(user_id, request.POST['password']) == None:
			return HttpResponse(json.dumps(db.userNotFoundError(), indent=4, separators=(',', ': ')),
			                    content_type="application/json")
		...
		if graph_exists == False:
			return HttpResponse(
				json.dumps(db.throwError(404, "User " + user_id + " owns no graph with id " + graphname + "!"),
				           indent=4, separators=(',', ': ')), content_type="application/json")
		else:
			return HttpResponse(
				json.dumps(db.sendMessage(200, "User " + user_id + " owns a graph with id " + graphname + "!"),
				           indent=4, separators=(',', ': ')), content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def upload_graph(request, user_id, graphname):
	...
	if request.method == 'POST':
		...
		if graph_errors != None:
			return HttpResponse(json.dumps(db.throwError(400, graph_errors), indent=4, separators=(',', ': ')),
			                    content_type="application/json")
		else:
			return HttpResponse(
				json.dumps(db.sendMessage(201, "Added " + graphname + " for " + user_id + '.'), indent=4,
				           separators=(',', ': ')), content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def update_graph(request, user_id, graphname):
	...
	if request.method == 'POST':
		...
		if graph_errors != None:
			return HttpResponse(json.dumps(db.throwError(404, graph_errors), indent=4, separators=(',', ': ')),
			                    content_type="application/json")
		else:
			return HttpResponse(
				json.dumps(db.sendMessage(201, "Updated " + graphname + " for " + user_id + '.'), indent=4,
				           separators=(',', ': ')), content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def retrieve_graph(request, user_id, graphname):
	...
	if request.method == 'POST':
		...
		if jsonData != None:
			...
			return HttpResponse(
				json.dumps(db.throwError(404, "No Such Graph Exists!"), indent=4, separators=(',', ': ')),
				content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def remove_graph(request, user_id, graphname):
	...
	if request.method == 'POST':
		...
		if jsonData != None:
			...
			return HttpResponse(
				json.dumps(db.sendMessage(200, "Successfully deleted " + graphname + " owned by " + user_id + '.'),
				           indent=4, separators=(',', ': ')), content_type="application/json")
		else:
			return HttpResponse(
				json.dumps(db.throwError(404, "No Such Graph Exists."), indent=4, separators=(',', ': ')),
...
def view_all_graphs_for_user(request, user_id):
	...
	if request.method == 'POST':
		...
		return HttpResponse(json.dumps({"StatusCode": 200, "Graphs": data}, indent=4, separators=(',', ': ')),
		                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def make_graph_public(request, user_id, graphname):
	...
	if request.method == 'POST':
		...
		if data == None:
			return HttpResponse(
				json.dumps(db.sendMessage(200, "Successfully made " + graphname + " owned by " + user_id + " public."),
				           indent=4, separators=(',', ': ')), content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.throwError(404, data), indent=4, separators=(',', ': ')),
			                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def make_graph_private(request, user_id, graphname):
	...
	if request.method == 'POST':
		...
		if data == None:
			return HttpResponse(
				json.dumps(db.sendMessage(200, "Successfully made " + graphname + " owned by " + user_id + " private."),
				           indent=4, separators=(',', ': ')), content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.throwError(404, data), indent=4, separators=(',', ': ')),
			                    content_type="application/json")
...
def get_groups(request):
	...
	if request.method == 'POST':
		...
		return HttpResponse(json.dumps({"StatusCode": 200, "Groups": data}, indent=4, separators=(',', ': ')),
		                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def get_group(request, group_owner, groupname):
	...
	if request.method == 'POST':
		...
		if data == None:
			return HttpResponse(
				json.dumps(db.throwError(404, "Group does not exist for this user!"), indent=4, separators=(',', ': ')),
				content_type="application/json")
...
def delete_group(request, group_owner, groupname):
	...
	if request.method == 'POST':
		...
		if group_owner == request.POST['username']:
			...
			if data != None:
				return HttpResponse(
					json.dumps(db.throwError(404, "Group not found!"), indent=4, separators=(',', ': ')),
					content_type="application/json")
			else:
				return HttpResponse(json.dumps(db.sendMessage(200, data), indent=4, separators=(',', ': ')),
				                    content_type="application/json");
		else:
			return HttpResponse(json.dumps(
				db.throwError(400, "The group owner and the person making this request are not the same person!"),
				indent=4, separators=(',', ': ')), content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def get_group_for_user(request, user_id):
	...
	if request.method == 'POST':
		...
		return HttpResponse(json.dumps({"StatusCode": 200, "Groups": group}, indent=4, separators=(',', ': ')),
		                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def add_user_to_group(request, group_owner, groupname, user_id):
	...
	if request.method == 'POST':
		...
		if group_owner == request.POST['username']:
			...
			if data == None:
				return HttpResponse(
					json.dumps(db.throwError(400, "Group doesn't exist or user has already been added!"), indent=4,
					           separators=(',', ': ')), content_type="application/json")
...
def remove_user_from_group(request, group_owner, groupname, user_id):
	...
	if request.method == 'POST':
		...
		if group_owner == request.POST['username']:
			...
			return HttpResponse(json.dumps(db.sendMessage(200, group), indent=4, separators=(',', ': ')),
			                    content_type="application/json")
		else:
			return HttpResponse(json.dumps(
				db.throwError(400, "The group owner and the person making this request are not the same person!"),
...
def share_graph(request, graphname, group_owner, groupname):
	...
	if request.method == 'POST':
		...
		if result == None:
			return HttpResponse(json.dumps(db.sendMessage(200, "Graph successfully shared with group!"), indent=4,
			                               separators=(',', ': ')), content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.sendMessage(400, result), indent=4, separators=(',', ': ')),
			                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def unshare_graph(request, graphname, group_owner, groupname):
	...
	if request.method == 'POST':
		...
		if result == None:
			return HttpResponse(json.dumps(db.sendMessage(200, "Graph successfully unshared with group!"), indent=4,
			                               separators=(',', ': ')), content_type="application/json")
		else:
			return HttpResponse(json.dumps(db.sendMessage(400, result), indent=4, separators=(',', ': ')),
			                    content_type="application/json")
...
def get_tags_for_user(request, username):
	...
	if request.method == 'POST':
		...
		return HttpResponse(json.dumps({"StatusCode": 200, "Tags": result}, indent=4, separators=(',', ': ')),
		                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def get_all_tags_for_graph(request, username, graphname):
	...
	if request.method == 'POST':
		...
		if result == None:
			return HttpResponse(
				json.dumps(db.sendMessage(404, "Graph does not exist!"), indent=4, separators=(',', ': ')),
				content_type="application/json")
		else:
			return HttpResponse(json.dumps({"StatusCode": 200, "Tags": result}, indent=4, separators=(',', ': ')),
			                    content_type="application/json")
...
def get_all_graphs_for_tags(request, tag):
	...
	if request.method == 'POST':
		...
		return HttpResponse(json.dumps({"StatusCode": 200, "Graphs": result}, indent=4, separators=(',', ': ')),
		                    content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def make_all_graphs_for_tag_public(request, username, tagname):
	...
	if request.method == 'POST':
		...
		if username == request.POST['username']:
			...
			if error == None:
				return HttpResponse(json.dumps(db.sendMessage(200, "Graphs with tag have been made public"), indent=4,
				                               separators=(',', ': ')), content_type="application/json")
			else:
				return HttpResponse(json.dumps(db.throwError(400, error), indent=4, separators=(',', ': ')),
				                    content_type="application/json")
		else:
			return HttpResponse(json.dumps(
				db.throwError(400, "The tag owner and the person making this request are not the same person!"),
				indent=4, separators=(',', ': ')), content_type="application/json")
	else:
		context = {"Error": "This route only accepts POST requests."}
		return render(request, 'graphs/error.html', context)
...
def make_all_graphs_for_tag_private(request, username, tagname):
	...
	if request.method == 'POST':
		...
		if username == request.POST['username']:
			...
			if error == None:
				return HttpResponse(json.dumps(db.sendMessage(200, "Graphs with tag have been made private"), indent=4,
				                               separators=(',', ': ')), content_type="application/json")
			else:
				return HttpResponse(json.dumps(db.throwError(400, error), indent=4, separators=(',', ': ')),
				                    content_type="application/json")
...
def delete_all_graphs_for_tag(request, username, tagname):
	...
	if request.method == 'POST':
		...
		if username == request.POST['username']:
			...
			return HttpResponse(
				json.dumps(db.sendMessage(200, "Graphs with tag have been deleted"), indent=4, separators=(',', ': ')),
				content_type="application/json")
		else:
			return HttpResponse(json.dumps(
				db.throwError(400, "The tag owner and the person making this request are not the same person!"),
...
def handler_404(request):
	if request.method == 'POST':
		return HttpResponse(json.dumps(db.throwError(404, "REST API endpoint does not exist!")),
		                    content_type="application/json")
	else:
...
def handler_500():
	if request.method == 'POST':
		return HttpResponse(json.dumps(
			db.throwError(500, "An error was encountered during this request.  REST API call not successful.")),
		                    content_type="application/json")
	else:
...
def constructGraphMessage(context, view_type, search, tags):
	if view_type == 'shared':
		if search == None and tags == None:
			...
			context[
				'message'] = "It appears that there are no groups that have shared their graphs with the given search criteria."
		elif tags != None and search == None:
			context[
				'message'] = "It appears that there are no groups that have shared their graphs with the given tag criteria."
		else:
			context[
				'message'] = "It appears that there are no groups that have shared their graphs with the given search and tag criteria."

	elif view_type == 'public':
		if search == None and tags == None:
			context[
				'message'] = "It appears that there are no public graphs available.  Please create an account and join a group or upload your own graphs through the <a href='/../help/programmers/#add_graph'>REST API</a> or <a href='upload'>web interface</a>."
		elif search != None and tags == None:
			context[
				'message'] = "It appears that there are no public graphs available that match the search criteria.  Please create an account and join a group or upload your own graphs through the <a href='/../help/programmers/#add_graph'>REST API</a> or <a href='upload'>web interface</a> with the given search criteria."
		elif tags != None and search == None:
			context[
				'message'] = "It appears that there are no public graphs available that match the tag criteria.  Please create an account and join a group or upload your own graphs through the <a href='/../help/programmers/#add_graph'>REST API</a> or <a href='upload'>web interface</a> with the given tag criteria."
		else:
			context[
				'message'] = "It appears that there are no public graphs available that match the search and tag criteria.  Please create an account and join a group or <a href='/../help/programmers/#add_graph'>upload</a> your own graphs with the given search and tag criteria."
		...
		if search == None and tags == None:
			context[
				'message'] = "It appears that you currently have no graphs uploaded. Please create an account and join a group or upload your own graphs through the <a href='/../help/programmers/#add_graph'>REST API</a> or <a href='upload'>web interface</a>."
		elif search != None and tags == None:
			context[
				'message'] = "It appears that you currently have no graphs uploaded that match the search terms. Please create an account and join a group or upload your own graphs through the <a href='/../help/programmers/#add_graph'>REST API</a> or <a href='upload'>web interface</a> with the given search criteria in order to see them here."
		elif tags != None and search == None:
			context[
				'message'] = "It appears that you currently have no graphs uploaded that match the tag terms. Please create an account and join a group or upload your own graphs through the <a href='/../help/programmers/#add_graph'>REST API</a> or <a href='upload'>web interface</a> with the given tag criteria in order to see them here."
		else:
			context[
