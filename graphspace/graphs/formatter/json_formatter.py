...
class GraphSpaceJSONFormat:
	...
	def create_gsgraph(json_string):
		...
		-------
		"""
		graph_json = GraphSpaceJSONFormat.validate_clean_json(GraphSpaceJSONFormat.clean_graph_json(json_string))
		G = GSGraph()
	...
	def clean_graph_json(original_json_string):
		"""
			...
			Changes to node attributes:
				1. If name is missing in the data attributes, it will copy id value to the name attribute.
			...
			Changes to edge attributes:
				...
				2. Add default target_arrow_shape value of none if not given by the user.
		...
		for node in old_nodes:
			...
			if 'id' in node['data'] and 'name' not in node['data']:
		...
		for edge in old_edges:
			...
			if 'target_arrow_shape' not in edge['style']:
				edge['style']['target_arrow_shape'] = "none"
