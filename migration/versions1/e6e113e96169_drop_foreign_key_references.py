...
Revision ID: e6e113e96169
Revises: 9a103ea8010b
Create Date: 2017-02-13 14:39:51.420832
...
revision = 'e6e113e96169'
down_revision = '9a103ea8010b'
branch_labels = None
depends_on = None
...
def upgrade():
	...
	op.drop_constraint('edge_user_id_fkey', 'group_to_user', type_='foreignkey')
...
def downgrade():
	# password_reset
	op.execute('ALTER TABLE "password_reset" ADD CONSTRAINT password_reset_user_id_fkey FOREIGN KEY (user_id) REFERENCES user (email) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE')

	# group
	op.execute('ALTER TABLE "group" ADD CONSTRAINT group_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES user (email) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE')

	# graph
	op.execute('ALTER TABLE "graph" ADD CONSTRAINT graph_user_id_fkey FOREIGN KEY (user_id) REFERENCES user (email) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE')
